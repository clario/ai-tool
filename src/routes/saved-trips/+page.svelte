		...
		{#if data.trips.length === 0}
			<div class="text-center py-16">
				...
				<p class="text-gray-500 mb-8 max-w-md mx-auto">
					Start planning your next adventure! Create a travel plan and save it to see it here.
				</p>
				<Button
							...
							{:else}
								...
								<div class="flex items-start justify-between">
									<div class="flex-1 min-w-0">
										<CardTitle
											class="text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors truncate"
										>
											{trip.name}
										</CardTitle>
										{#if trip.description}
								...
								<div class="flex gap-2">
									...
									{#if showDeleteConfirm === trip.id}
										<div class="flex gap-1">
											<Button onclick={() => cancelDelete()} variant="outline" class="px-3">
												Cancel
											</Button>
											<form method="POST" action="?/deleteTrip" use:enhance={handleDelete}>
												<input type="hidden" name="tripId" value={trip.id} />
												<Button
													type="submit"
													variant="destructive"
